...
`vba-insight` scans container packages (OOXML relationships, embedded OLE parts), workbook cell formulas, defined names, and worksheet metadata against 61 dedicated security rules:
...
| `VBA-CELL-050` | `SlicerOrTimelineCacheAnomaly` | **Critical** | Excel Slicer or Timeline definitions (`xl/slicers/`, `xl/timelines/`) or relationships contain remote UNC connection paths enabling NTLM credential coercion, dangerous exploit URI schemes (`ms-msdt:`, `search-ms:`, `mhtml:`, `javascript:`, `powershell:`), executable/script targets (`.exe`, `.bat`, `.vbs`, `.ps1`, `.hta`), database command execution (`xp_cmdshell`, `sp_OACreate`, `OPENROWSET`), or host environment profiling tokens (`%USERNAME%`). |
| `VBA-CELL-051` | `BibliographyOrCitationAnomaly` | **Critical** | Word Bibliography and citation definitions (`word/bibliography/sources.xml`, `customXml/`) contain remote UNC paths enabling NTLM credential coercion, dangerous exploit URI schemes (`ms-msdt:`, `search-ms:`, `ms-appinstaller:`, `mhtml:`, `powershell:`), shell execution commands (`powershell`, `cmd.exe`, `wscript.exe`, `cscript.exe`, `mshta`, `rundll32`, `certutil`), or smuggled Windows PE binaries. |
| `VBA-CELL-052` | `CustomXmlDataBindingOrXPathAnomaly` | **Critical** | Structured Document Tags (SDT) or Custom XML data bindings (`<w:dataBinding>`, `<m:dataBinding>`, `<p:dataBinding>`) configure XPath queries containing external document resolution functions (`document()`, `doc()`) enabling SSRF / NTLM credential theft, database/shell command injection, or remote UNC / exploit protocol namespace declarations in `prefixMappings` or `storeItemID`. |
| `VBA-CELL-053` | `XmlMapsOrSchemaDefinitionAnomaly` | **Critical** | Excel XML Map definitions (`xl/xmlMaps.xml`), table XML column mappings (`xl/tables/table*.xml`), or relationships contain remote UNC paths enabling NTLM credential coercion, dangerous exploit URI schemes (`ms-msdt:`, `search-ms:`, `ms-appinstaller:`, `mhtml:`, `powershell:`), XXE DTD/entity declarations, smuggled Windows PE executables, shell commands (`powershell`, `cmd.exe`, `wscript.exe`), or table column XPath SSRF document functions (`document()`, `doc()`). |
| `VBA-CELL-054` | `CommentAnnotationOrAuthorAnomaly` | **Critical** | Document comment and modern annotation parts (`word/comments*.xml`, `ppt/comments/*.xml`, `ppt/modernComments/*.xml`, `xl/comments*.xml`, `xl/threadedComments/*.xml`) or relationships contain remote UNC paths enabling NTLM credential coercion, dangerous exploit URI schemes, smuggled Windows PE executables, shell execution commands (`powershell`, `cmd.exe`, `mshta`, `rundll32`), or executable payload targets (`.exe`, `.bat`, `.ps1`, `.hta`, `.lnk`). |
| `VBA-CELL-055` | `ThemeFontOrEffectCoercionAnomaly` | **Critical** | Office Theme parts (`*/theme/theme*.xml`, `themeOverride*.xml`) or relationships contain remote UNC paths in font typeface definitions (`<a:latin typeface="\\..."/>`, `<a:ea typeface="\\..."/>`, `<a:cs typeface="\\..."/>`) enabling NTLM coercion or font engine exploits, dangerous exploit URI schemes, external theme relationships linking to remote templates/themes, or smuggled Windows PE binaries. |
| `VBA-CELL-056` | `CustomXmlPropertiesOrItemSchemaAnomaly` | **Critical** | Custom XML item properties (`customXml/itemProps*.xml`), data parts, or relationships contain remote UNC paths in schema references (`ds:schemaRef`, `targetNamespace`, `schemaRef`) enabling NTLM credential coercion, dangerous exploit URI schemes (`ms-msdt:`, `search-ms:`, `ms-appinstaller:`, `mhtml:`, `powershell:`), external relationships to executables, or smuggled Windows PE binaries. |
| `VBA-CELL-057` | `VbaDataStreamOrProjectRelsAnomaly` | **Critical** | VBA project relationships (`xl/_rels/vbaProject.bin.rels`, `vbaProjectSignature*.bin.rels`) or supplementary VBA data streams (`xl/vbaData.xml`) contain external relationships linking to remote UNC paths or binaries, dangerous exploit URI schemes, smuggled Windows PE executables, or shell execution commands. |
| `VBA-CELL-058` | `WordGlossaryOrBuildingBlocksRelsAnomaly` | **Critical** | Word glossary definitions (`word/glossary/settings.xml`, `document.xml`) or building blocks relationships configure remote template injection via `attachedTemplate`, remote UNC paths enabling NTLM credential coercion, dangerous exploit URI schemes, external relationships to executables, or smuggled Windows PE binaries. |
| `VBA-CELL-059` | `WordDocVariablesOrNotesAnomaly` | **Critical** | Word document variables (`word/settings.xml` `<w:docVars>`), footnotes (`word/footnotes.xml`), or endnotes (`word/endnotes.xml`) contain smuggled Windows PE binaries, shell execution commands, dangerous exploit URI schemes (`ms-msdt:`, `search-ms:`, `mhtml:`, `powershell:`), remote UNC paths enabling NTLM credential coercion, or external relationships targeting executable or script payloads. |
| `VBA-CELL-060` | `PowerPointTagsOrMastersAnomaly` | **Critical** | PowerPoint programmable tags (`ppt/tags/tag*.xml`), presentation relationships, font table, handout master, or notes master contain smuggled Windows PE binaries, shell execution commands, dangerous exploit URI schemes, remote UNC paths enabling NTLM credential coercion, or external relationships targeting executable or script payloads. |
| `VBA-CELL-061` | `ScenarioManagerOrConsolidationAnomaly` | **Critical** | Excel Scenario Manager definitions (`xl/scenarios/`, `<scenarios>`), scenario input cells, or workbook consolidation references contain cloaked DDE execution commands, Excel 4.0 (XLM) macro execution functions (`EXEC(`, `CALL(`), shell execution commands, smuggled Windows PE binaries, or remote UNC workbook paths in data consolidation enabling NTLM credential coercion. |

- **Container-Level Threat Inspection**: Automatically scans OOXML package relationships, drawings, data connections, embedded SVG images, digital signatures, Word field codes (`w:fldSimple`, `w:instrText`), PowerPoint slide actions, AltChunk format payload smuggling parts, Custom UI Ribbon XML callbacks, legacy dialog sheets, `[Content_Types].xml` MIME declarations, external link caches (`xl/externalLinks/`), Word web settings framesets (`word/webSettings.xml`), workbook protection cloaking structures (`xl/workbook.xml`), smuggled executable/script payloads or cloaked PE headers (`VBA-CELL-032`), worksheet view evasion / hidden grid cloaking (`VBA-CELL-033`), ActiveX object CLSID / property declarations (`VBA-CELL-034`), Word glossary document injection (`VBA-CELL-035`), font payload smuggling and ODTTF obfuscation (`VBA-CELL-036`), digital ink action anomalies (`VBA-CELL-037`), document property payload smuggling (`VBA-CELL-038`), web extension/taskpane auto-show (`VBA-CELL-039`), PivotCache data connection anomaly (`VBA-CELL-040`), metafile exploit records and PE cloaking (`VBA-CELL-041`), XSLT transform and MSXSL script injection (`VBA-CELL-042`), relationship target cloaking or evasion (`VBA-CELL-043`), SmartArt and diagram action/payload smuggling (`VBA-CELL-044`), Word MailMerge coercion and query injection (`VBA-CELL-045`), Excel QueryTable external query abuse (`VBA-CELL-046`), Power Query M formulas and Data Mashup parts (`VBA-CELL-047`), OLE object Moniker and auto-activation / icon cloaking (`VBA-CELL-048`), XML namespace cloaking / schema spoofing (`VBA-CELL-049`), Excel Slicer and Timeline cache anomalies (`VBA-CELL-050`), Word bibliography and citation anomalies (`VBA-CELL-051`), Custom XML SDT data binding / XPath injection (`VBA-CELL-052`), XML Maps and table schemas (`VBA-CELL-053`), document comments and modern annotations (`VBA-CELL-054`), theme fonts and remote template coercion (`VBA-CELL-055`), custom XML properties and schema anomalies (`VBA-CELL-056`), VBA project relationships and data stream anomalies (`VBA-CELL-057`), Word glossary building blocks relationships (`VBA-CELL-058`), Word document variables and notes (`VBA-CELL-059`), PowerPoint programmable tags and master slides (`VBA-CELL-060`), and Excel Scenario Manager and consolidation sources (`VBA-CELL-061`) for Remote Template Injection, embedded OLE packager binaries, external Moniker links, ActiveX controls, printer settings UNC coercion, custom XML payload smuggling, drawing hover/macro actions, data connections / NTLM coercion, SVG script vectors, signature tampering/stripping, and dangerous protocol handlers even in macro-less weaponized containers.
- **Dynamic Formula De-Obfuscation**: Evaluates complex obfuscated formulas across the workbook grid with complex numbers and difference sums (`COMPLEX()`, `IMREAL()`, `IMAGINARY()`, `IMABS()`, `IMCONJG()`, `SUMXMY2()`, `SUMX2MY2()`, `SUMX2PY2()`, `MULTINOMIAL()`), matrix determinant and inversion (`MDETERM()`, `MINVERSE()`), flooring and polynomial series (`INT()`, `SERIESSUM()`), matrix algebra (`MMULT()`, `MUNIT()`), reciprocal trigonometry and hyperbolics (`SEC()`, `CSC()`, `COT()`, `SECH()`, `CSCH()`, `COTH()`), inverse cotangents (`ACOT()`, `ACOTH()`), hyperbolic and inverse hyperbolic trigonometry (`SINH()`, `COSH()`, `TANH()`, `ASINH()`, `ACOSH()`, `ATANH()`), square root with $\pi$ (`SQRTPI()`), sum of squares (`SUMSQ()`), mathematical constants and exponentials (`PI()`, `EXP()`), logarithms (`LN()`, `LOG10()`, `LOG()`), combinatorics and permutations (`COMBIN()`, `PERMUT()`), multi-array inner product summation (`SUMPRODUCT()`), dynamic sequence generation (`SEQUENCE()`), implicit intersection extraction (`SINGLE()`), Excel 365 dynamic array stacking (`VSTACK()`, `HSTACK()`), logical branching (`IFS()`, `SWITCH()`, `XOR()`), lambda helper functions (`MAP()`, `REDUCE()`, `SCAN()`, `BYROW()`, `BYCOL()`, `MAKEARRAY()`, `ISOMITTED()`), native array literals (`{...}`), custom lambda functions (`LAMBDA()`), scoped variable evaluation (`LET()`), dynamic URL encoding (`ENCODEURL()`), radix conversions (`BIN2HEX()`, `HEX2BIN()`, `OCT2HEX()`, `HEX2OCT()`, `BASE()`, `DECIMAL()`, `HEX2DEC()`, `BIN2DEC()`), locale-independent parsing (`NUMBERVALUE()`), recursive reference resolution (`INDIRECT()`, `OFFSET()`, `ADDRESS()`), cross-cell formula inspection (`FORMULATEXT()`), dynamic array filtering and sorting (`FILTER()`, `SORT()`, `SORTBY()`, `UNIQUE()`, `WRAPROWS()`, `WRAPCOLS()`), bitwise decryption (`BITXOR()`, `BITAND()`, `BITOR()`, `BITLSHIFT()`, `BITRSHIFT()`), Roman numeral conversions (`ROMAN()`, `ARABIC()`), modern dynamic array reshaping (`TAKE()`, `DROP()`, `CHOOSEROWS()`, `CHOOSECOLS()`, `TOROW()`, `TOCOL()`, `EXPAND()`), modern dynamic lookups (`XLOOKUP()`, `XMATCH()`), array inversion (`TRANSPOSE()`), vector/array lookup (`LOOKUP()`), branchless conditional evaluation (`DELTA()`, `GESTEP()`, `SIGN()`), DBCS double-byte character slicing (`LENB()`, `LEFTB()`, `RIGHTB()`, `MIDB()`), integer arithmetic and even/odd rounding (`QUOTIENT()`, `EVEN()`, `ODD()`), factorial calculations (`FACT()`, `FACTDOUBLE()`), number-theoretic reductions (`GCD()`, `LCM()`), text dissection and serialization (`TEXTBEFORE()`, `TEXTAFTER()`, `TEXTSPLIT()`, `ARRAYTOTEXT()`, `VALUETOTEXT()`), multi-cell range aggregations (`CONCAT()`, `TEXTJOIN()`), 2D table lookups (`INDEX()`, `VLOOKUP()`, `HLOOKUP()`, `MATCH()`), Unicode conversions (`UNICHAR()`, `UNICODE()`), and mathematical/string operations (`CHAR()`, `MID()`, `SUBSTITUTE()`, `CHOOSE()`, `HYPERLINK()`, `ROWS()`, `COLUMNS()`, `MROUND()`, `TYPE()`, `ISNONTEXT()`) to uncover hidden LOLBins, multi-cell DDE execution, RTD COM automation, and remote download payloads that evade static pattern matching.

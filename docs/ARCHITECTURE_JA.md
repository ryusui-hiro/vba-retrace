...
- ワークシートの危険なセル数式スキャン（`extract::extract_xlsm` / `extract::scan_workbook_cell_threats`）は、OOXMLコンテナ内のワークシートXML（`/xl/worksheets/sheet*.xml`）および外部リレーション（`_rels`）を検証し、DDE実行（`VBA-CELL-001`）、Excel 4.0マクロ数式（`VBA-CELL-002`）、外部リモートブックリンク（`VBA-CELL-003`）、WEBSERVICE / FILTERXMLによる外部データ取得・持ち出し数式（`VBA-CELL-004`）、不審な実行ファイルダウンロードハイパーリンク（`VBA-CELL-005`）、Auto_Open等の自動実行定義名（`VBA-CELL-006`）、`state="veryHidden"`によるシート不可視化回避（`VBA-CELL-007`）、レガシーXLMマクロシート（`VBA-CELL-008`）、および難読化数式評価による動的脅威復元（`VBA-CELL-009`、`CHAR`・`UNICHAR`・`HEX2DEC`・`BIN2DEC`・`INDEX`・`VLOOKUP`・`TEXTJOIN`・`CONCATENATE`・`MID`・`SUBSTITUTE`・`CHOOSE`・`HYPERLINK` 有界評価）を検出して構造化報告に記録します。
- エクスポート層（`export::inspection_to_sarif`, `export::stomping_to_sarif`）は、OASIS SARIF v2.1.0標準に準拠したセキュリティレポートを生成します。VBA Stomping（`VBA-STOMP-001..010`）およびセル脅威（`VBA-CELL-001..009`）の全ルール定義、CWEマッピング、詳細ヘルプMarkdown、URI物理ロケーション情報を出力し、GitHub Advanced Security / Code Scanning および CI セキュリティゲートとの完全な相互運用を実現しています。

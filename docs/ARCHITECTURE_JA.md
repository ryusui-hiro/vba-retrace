...
- ワークシートおよびコンテナパッケージの脅威スキャン（`extract::extract_xlsm` / `extract::extract_macro_container` / `extract::scan_ooxml_package_threats`）は、OOXMLコンテナ内のワークシートXML（`/xl/worksheets/sheet*.xml`）、外部リレーション（`_rels`）、およびコンテナパーツ構造を検証し、DDE実行（`VBA-CELL-001`）、Excel 4.0マクロ数式（`VBA-CELL-002`）、外部リモートブックリンク（`VBA-CELL-003`）、WEBSERVICE / FILTERXMLによる外部データ取得・持ち出し数式（`VBA-CELL-004`）、不審な実行ファイルダウンロードハイパーリンク（`VBA-CELL-005`）、Auto_Open等の自動実行定義名（`VBA-CELL-006`）、`state="veryHidden"`によるシート不可視化回避（`VBA-CELL-007`）、レガシーXLMマクロシート（`VBA-CELL-008`）、難読化数式評価による動的脅威復元（`VBA-CELL-009`、グリッド動的解決 `INDIRECT`・`OFFSET`・`ADDRESS`、ビット演算暗号復号 `BITXOR`・`BITAND`・`BITOR`・`BITLSHIFT`・`BITRSHIFT`、最新動的検索 `XLOOKUP`・`XMATCH`、テキスト分解 `TEXTBEFORE`・`TEXTAFTER`・`TEXTSPLIT`、複数セル範囲集約 `CONCAT`・`TEXTJOIN`、2Dテーブル参照 `INDEX`・`VLOOKUP`・`HLOOKUP`・`MATCH`、基数・Unicode変換 `HEX2DEC`・`BIN2DEC`・`UNICHAR`、文字列・数学関数 `CHAR`・`MID`・`SUBSTITUTE`・`CHOOSE`・`HYPERLINK` 有界評価）、リモートテンプレートインジェクション（`VBA-CELL-010`）、埋め込み OLE パッケージ / 数式エディタ攻撃（`VBA-CELL-011`）、外部 OLE オブジェクトリンク（`VBA-CELL-012`）、埋め込み ActiveX コントロール（`VBA-CELL-013`）、および外部サブドキュメント・フレーム参照（`VBA-CELL-014`）を網羅的に検出して構造化報告に記録します。マクロコードが存在しない武器化 DOCX/XLSX コンテナであってもパッケージレベル脅威を正確に検知可能です。
- エクスポート層（`export::inspection_to_sarif`, `export::stomping_to_sarif`）は、OASIS SARIF v2.1.0標準に準拠したセキュリティレポートを生成します。VBA Stomping（`VBA-STOMP-001..010`）およびコンテナ・セル脅威（`VBA-CELL-001..014`）の全ルール定義、CWEマッピング、詳細ヘルプMarkdown、URI物理ロケーション情報を出力し、GitHub Advanced Security / Code Scanning および CI セキュリティゲートとの完全な相互運用を実現しています。

...
`vba-insight` 针对容器包结构（OOXML 关系、内嵌 OLE 部件）、工作簿公式、定义名及工作表元数据进行 55 项专业安全规则筛查：
